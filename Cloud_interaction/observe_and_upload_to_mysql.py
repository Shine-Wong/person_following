...
lower_shang = np.array([120, 30, 30])  #the range of red_clothes
upper_shang = np.array([190, 255, 255])

lower_xia = np.array([0, 0, 0])  #the range of black
...
def get_terminal_and_writedata(s_t, a_t, r_t, s_t1, s_t_orig, s_t1_orig, camera):
    ...
    try:
        ...
        s_t_json,a_t_json,s_t1_json=array_to_json(s_t,a_t,s_t1)
        # query = """INSERT INTO list_d VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"""
        ...
        arg = (data_num, s_t_json, a_t_json, r_t, s_t1_json,
               terminal)
        print("data_num is:{},s_t_json len is:{},s_t1_json len is:{},r_t is:{},terminal is:{}".format(
            data_num, len(s_t_json), len(s_t1_json), r_t, terminal))
...
def trainNetwork(s, readout, h_fc1, keep_prob, sess):
    ...
    while "flappy bird" != "angry bird":
        ...
        s_t1_orig = np.append(x_t1_orig, s_t_orig[:, :, 3:12], axis=2)
